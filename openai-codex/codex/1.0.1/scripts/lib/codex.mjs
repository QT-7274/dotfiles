...
export function getCodexAvailability(cwd) {
  ...
  // Detect codex-internal where app-server exists but is disabled at runtime
  const appServerOutput = (appServerStatus.detail || "").toString();
  const appServerDisabled = !appServerStatus.available ||
    appServerOutput.includes("内部禁用") ||
    appServerOutput.includes("命令已禁用") ||
    !appServerOutput.includes("app-server");
  if (appServerDisabled) {
    // Fallback for codex-internal where app-server is disabled:
    // still mark as available so review can attempt direct `codex review`
    return {
      available: true,
      detail: `${versionStatus.detail}; advanced runtime unavailable (fallback mode)`
    };
  }
...
export function getCodexLoginStatus(cwd) {
  ...
  // Fallback: for codex-internal where "login status" is disabled,
  // check if the CLI can actually run by invoking --version
  const fallback = runCommand("codex", ["--version"], { cwd });
  if (fallback.status === 0 && fallback.stdout.trim()) {
    return {
      available: true,
      loggedIn: true,
      detail: "authenticated (codex-internal fallback)"
    };
  }
...
export async function runAppServerReview(cwd, options = {}) {
  ...
  // Fallback for codex-internal: app-server is disabled, use direct `codex review` CLI
  if (availability.detail && availability.detail.includes("fallback mode")) {
    emitProgress(options.onProgress, "Using direct codex review (app-server unavailable).", "starting");
    const args = ["review"];
    if (options.target) {
      // nativeTarget format: { type: "baseBranch", branch } or { type: "uncommittedChanges" }
      if (options.target.type === "baseBranch" && options.target.branch) {
        args.push("--base", options.target.branch);
      } else if (options.target.type === "uncommittedChanges") {
        args.push("--uncommitted");
      }
    }
    const result = runCommand("codex", args, { cwd });
    // codex review writes all output to stderr, not stdout
    const stdout = (result.stdout || "").trim();
    const stderr = (result.stderr || "").trim();
    const reviewText = stdout || stderr || "No review output.";
    return {
      ...
      turn: null,
      error: result.status !== 0 ? stderr : null,
